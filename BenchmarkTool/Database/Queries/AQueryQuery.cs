...
{
    public class AQueryQuery : IQuery
    {
        private static string _rangeAgg = @"SELECT {1} AS time_agg, sensor_id, avg({2}) FROM {3}
        ...
        private static string _outOfRange = @"SELECT {1} AS time_agg, max({2}), min({2}) FROM {3}
        ...
        private static string _aggDifference = @"SELECT A1.time_agg, A2.val - A1.val as difference from
                                                    (SELECT {1} AS time_agg , avg({2}) as val FROM {3}
